...
public class PharusClient extends PApplet 
{
  ...
  public void pre() 
  {
    ...
    for (TuioObject _to : tuioObjectList)
    {
      Player p = null;
      if (_to.getSymbolID() < tuioCursorList.size())
      {
        TuioCursor tc = tuioCursorList.get(_to.getSymbolID());
        p = players.get(tc.getSessionID());
      }
      if (p != null)
      {
        p.feet.add(new Foot(_to.getScreenX(width), _to.getScreenY(height - wallHeight) + wallHeight));
      }
      else
      {
        println("unkown foot id: " + _to.getSymbolID());
      } 
    }
